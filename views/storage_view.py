...
class StorageView:
    ...
    def show_location_details(self, row, col):
        ...
        self.warehouse.data_storage.save_log(self.warehouse.user,
            f"Viewed location {location.get_location_code()}")
    ...
    def store_product_action(self):
        ...
        if self.warehouse.store_product(sku, quantity, row, col):
            # log store
            self.warehouse.data_storage.save_log(self.warehouse.user,
                f"Stored SKU {sku}, qty {quantity} at {self.row_var.get()}{self.col_var.get()}")
    ...
    def retrieve_product_action(self):
        ...
        if self.warehouse.retrieve_product(sku, quantity, row, col):
            # log retrieve
            self.warehouse.data_storage.save_log(self.warehouse.user,
                f"Retrieved SKU {sku}, qty {quantity} from {location_code}")
